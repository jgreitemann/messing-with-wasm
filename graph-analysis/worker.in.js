...
var Module = {
    ...
    postRun: function () {
        ...
        get_fiedler = Module.cwrap('get_fiedler', 'number', ['number', 'number']);
        ...
        compute_bias_histo = Module.cwrap('compute_bias_histo', null, ['number', 'number']);
        compute_weight_histo = Module.cwrap('compute_weight_histo', null, ['number', 'number']);
        ...
        self.onmessage = function(event) {
            ...
            if (msg.action == 'queue_update') {
                ...
                if (!msg.calc_fiedler) {
                    ...
                    compute_bias_histo(bias_ptr, bias_histo_ptr);
                    bias_histo_data = [];
                    for (var i = 0; i < 50; ++i)
                        bias_histo_data.push(Module.getValue(bias_histo_ptr + 4 * i, 'i32'));
                    compute_weight_histo(weight_ptr, weight_histo_ptr);
                    weight_histo_data = [];
                    for (var i = 0; i < 50; ++i)
                    ...
                    var degen = get_fiedler(weight_ptr, fiedler_ptr);
